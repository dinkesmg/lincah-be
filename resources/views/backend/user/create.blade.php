...
@section('page-script')
    ...
    $(document).ready(function () {
        ...
        function toggleKelurahanField() {
            let selectedRoles = $('select[name="role[]"]').val() || [];
            let kelurahanRow = $('#kelurahan-row');

            if (selectedRoles.includes('PUSKESMAS')) {
                kelurahanRow.slideDown();
            } else {
                kelurahanRow.slideUp();
                $('select[name="kelurahan_id[]"]').val(null).trigger('change');
            }
        }
    });
    </script>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form
                        ...
                        <div class="row mb-3" id="kelurahan-row" style="display:none">
                            <label class="col-sm-2 col-form-label" for="kelurahan_id">Kelurahan</label>
                            <div class="col-sm-10">
                                {{ html()->select('kelurahan_id[]', 
                                    $kelurahan->mapWithKeys(function ($item) {
                                        return [$item->id => "{$item->kecamatan->nama} - {$item->nama}"];
                                    }),
                                    old('kelurahan_id', $kelurahanIds ?? [])
                                )
