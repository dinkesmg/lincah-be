...
<body>
    ...
    <script>
        $(document).ready(function() {
            // Cari semua elemen label yang memiliki atribut `for`
            $('label[for]').each(function() {
                var label = $(this);
                var inputId = label.attr('for');
                var inputElement = $('#' + inputId);

                // Periksa apakah elemen input memiliki atribut `required`
                if (inputElement.is('[required]')) {
                    // Tambahkan bintang merah setelah teks label
